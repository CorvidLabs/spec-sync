...
- `REQ-change-007`: `malformed_policy_fails_closed`, `unavailable_required_path_coverage_fails_closed`, `archive_waits_until_delivery_diff_no_longer_needs_coverage`, `oversized_change_artifacts_are_rejected`, `safe_project_paths_reject_symlink_escapes`, and `unified_gate_validates_code_against_effective_delta`.
...
The complete `change::tests` matrix also passes with `CI=true`, `GITHUB_ACTIONS=true`, and a repository-level `GITHUB_BASE_REF`, proving that disposable Git fixtures do not inherit unrelated pull-request base state. A clean initialized consumer outside `GITHUB_WORKSPACE` passes the same strict Action command with 100% empty-project coverage. The executable five-epic demo creates a 16-commit product history and proves five definition approvals, five closing approvals, five verification records, ordered dependencies, post-merge archival, six permanent requirements, six passing product tests, 100% file/LOC coverage, and an A/100 spec score.
