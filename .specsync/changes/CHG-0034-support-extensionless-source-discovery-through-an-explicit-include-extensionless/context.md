...
Issue #369 requires an explicit, backwards-compatible way to include extensionless source files. The setting must affect every source-discovery path, not only the coverage command, while leaving omitted and empty extension lists unchanged. Wizard discovery must also exclude matching directories before applying filename and extension rules.
