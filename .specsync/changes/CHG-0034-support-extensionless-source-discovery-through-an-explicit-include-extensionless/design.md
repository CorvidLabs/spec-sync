...
Introduce one shared source-extension predicate that first recognizes a path with no extension when the boolean is enabled, then delegates ordinary extension matching to the existing predicate. All configuration-driven scanners use this shared predicate. Dotfiles follow `std::path::Path::extension` semantics and therefore count as extensionless. Directory and exclusion filtering remain unchanged, and wizard matching explicitly requires a regular file before applying module-name and extension rules.
