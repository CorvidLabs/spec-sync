...
The final release-branch reconciliation advances CHG43's definition-bound base and legacy cutoff to the released `v5.1.0` marker. The baseline therefore includes the byte-identical CHG42 release archive introduced on main before that marker, while preserving the rule that nothing introduced after the approved cutoff can inherit legacy trust.

Strict project checks authenticate the definition-bound baseline authority once and capture all explicitly scoped legacy archive subtrees in one bounded immutable evidence generation. Each archived record still receives its own path, subtree-digest, introduction, and history validation, but the corpus no longer repeats repository detection, index inspection, attributes, and payload capture for every archive. Explicit archive scopes preserve tracked sparse-absent entries even though the broader `.specsync/archive/` tree remains volatile for ordinary project-input discovery.

The same command-level snapshot rule applies to active verifying changes: one stable project-input digest is reused across their contract and persistence checks. This keeps all records in a single strict invocation on the same immutable project view and avoids repeating the full-repository capture for every verifying record.
