...
- `REQ-change-038`: unit coverage proves a multi-entry batch appends contiguous sequences in one
  write, and that an invalid later entry leaves prior state bytes unchanged.
- `REQ-change-038`: unit coverage proves `--all-missing` selects only production-source affected
