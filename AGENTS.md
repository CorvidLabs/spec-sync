...
4. **Never merge with active changes** — merge only after every change on the PR is archived. If you merge first, run `change status <id>` on `main`, then `change audit --strict`, and follow the exact state-specific recovery it reports. Do not assume every workflow-v2 record can start with `check --commit`: an `Accepted` record resumes through `change finalize` or `change ship`, while a `Verifying` record may need a fresh check before review and ship. If history is unavailable, re-record the review anchor before treating the change as current. Workflow-v1 records use their explicit `verify` → `accept` → merge → archive handoff. A green audit does not replace the missing lifecycle steps.
