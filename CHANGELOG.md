...
- **Draft specs can map planned source files** — safe missing paths in `draft` specs now produce non-failing planned-mapping notices and remain outside current file and LOC coverage. Creating the file or activating the spec restores normal validation, while `require_draft_files = true` keeps immediate existence enforcement available for strict repositories.
