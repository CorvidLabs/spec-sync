...
- Pin SpecSync in CI to the exact release tag `v6.0.0` with `version: '6.0.0'` (the README form) or a commit SHA. A floating major ref (`uses: CorvidLabs/spec-sync@v6`) is the compatibility channel; omitted `version` downloads `6.0.0`. Avoid `version: latest`
