...
not pin `GIT_CEILING_DIRECTORIES` on the default path: a project whose root is a subdirectory
of a git repository must still discover that work tree. Inherited git overrides are already wiped
by the cleared environment. MCP snapshot tool and resource dispatch pin
`GIT_CEILING_DIRECTORIES` to the snapshot parent for the duration of git probes, so a tempfile
sitting inside a host worktree (`TMPDIR` pointing at the repo) cannot walk up into that
worktree. CLI callers never enter that wrap.
