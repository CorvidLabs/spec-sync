...
- `REQ-change-032`: unit and CLI integration coverage corrects both supported fields in both directions, preserves original state and superseded evidence, validates portable append-only digest chains, adds artifacts monotonically, rejects malformed/no-op/unsupported/non-accepted corrections without mutation, requires fresh gates, supports a second correction only after reacceptance, and proves canonical deltas never replay.
