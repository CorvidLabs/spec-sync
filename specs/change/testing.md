...
- `REQ-change-038`: unit coverage proves a multi-entry exact-owner batch appends contiguous sequences in one write, an invalid later entry leaves prior state bytes unchanged, and `--all-missing` discovery selects only production-source affected paths that lack canonical owners.
