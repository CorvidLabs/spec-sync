...
- Ships prebuilt binaries for Linux and macOS; Windows is not a supported target as of 6.0 (use WSL)
- Must stay correct for repositories authored on any host platform, Windows included: CRLF line
  endings, Windows-reserved filenames, Windows-invalid characters, and backslash path separators
  reach Linux and macOS readers through any teammate's clone, so that handling is scoped to the
  content and not to the published binary set
...
The `cli` module SHALL preserve truthful user-visible behavior for the pre-6.0 product fixes landed in this change.
