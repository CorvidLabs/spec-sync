...
Unified JSON checking SHALL preserve the documented top-level check schema when SDD validation or coverage discovery fails.
...
- Malformed manifest discovery exits nonzero and emits valid JSON with `passed: false`, `valid: false`, `inconclusive: true`, and an explicit error.
