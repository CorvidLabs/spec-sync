...
| `tests/integration.rs` (--fix) | cargo test --test integration fix_ | `fix_adds_undocumented_exports_to_spec`, `fix_does_not_duplicate_already_documented_exports`, `fix_creates_public_api_section_when_missing`, `fix_with_json_output`, `fix_does_not_duplicate_when_non_export_subsections_present`, `fix_near_miss_handles_levenshtein_typos`, `fix_dry_run_does_not_write_files`, `fix_backup_creates_backup_dir`, `fix_backup_preserves_original_on_success`, `fix_inserts_row_into_table_under_bold_label_before_trailing_prose`, `fix_inserts_row_into_table_under_heading_subsection`, `fix_creates_rows_at_section_end_when_no_table_exists`, `fix_ignores_pipe_lines_inside_fenced_examples_after_the_table`, `fix_takes_column_count_from_the_real_table_not_a_preceding_code_sample`, `fix_honours_fence_delimiter_length_for_nested_fences`, `fix_ignores_fenced_exported_heading_before_the_table`, `fix_preserves_fenced_example_when_renaming_a_near_miss_header` (#615: rows extend the existing table, never follow trailing prose; fenced examples survive header rename) |
...
| Auto-fix extends an existing table (#615) | Public API table under a `**Bold**` label or `### Heading` followed by trailing prose | `cmd_check` runs with `--fix`, then `check --strict --force` | the row lands directly after the last table row, the prose is preserved, and strict passes (`fix_inserts_row_into_table_under_bold_label_before_trailing_prose`, `fix_inserts_row_into_table_under_heading_subsection`); with no table, rows still go at the section end (`fix_creates_rows_at_section_end_when_no_table_exists`) |
