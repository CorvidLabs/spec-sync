...
| Checked selected config | focused `issues_*config*` integration tests plus `load_issues_config_checked_with_hooks` unit tests | Malformed/invalid UTF-8/wrong-shaped, linked, replaced, FIFO-substituted, or over-4-MiB config emits a structured finding without blocking; exact retained bytes and one project capability govern parsing and specs. |
| Retained omitted-source discovery | ignored-name symlink plus manifest replacement/FIFO regressions | Ignored names are skipped before target inspection; no-follow, non-blocking retained-handle acquisition rejects recognized special files and replacement identities with a configuration finding. |
