...
- **Pre-populate exports**: exported symbols from detected sources are pulled via `generator::generate_spec` / `collect_exports_for_files`, de-duplicated, and seeded into the Public API table so the author starts from the real surface area.
- **Never clobber**: an existing target spec aborts with exit 1 — creation is non-destructive.
- **No chrono dependency**: Change Log dates come from the shared generator, not an in-module `chrono_lite_today()` helper.

## Files to Read First

- `src/commands/new.rs` — the command itself: `cmd_new` and `detect_module_sources`.
- `src/generator.rs` — `generate_spec`, `collect_exports_for_files`, `generate_companion_files_for_spec`.
- `src/exports/mod.rs` — `has_configured_extension` and `is_test_file`, used for source detection.
