...
| Metadata-descendant provenance | `python3 .github/scripts/test-reuse-check-from-ancestors.py` and `python3 .github/scripts/test-verify-trusted-policy-check.py` | First-parent bounds, second-parent exclusion, exact historical scoped-review and parent-bound workflow-v2 archive edges, metadata-check skipping, terminal product boundaries, exact check/job/run identity, complete check lookup, foreign and malformed rejection, unsuccessful-only failure, and success preference over newer cancellation/failure |
