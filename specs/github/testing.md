...
|------|---------|---------------------|
| `src/github.rs` | cargo test github:: | URL parsing, injected prepare/fetch orchestration, per-spec attribution, cap-before-provider, full deadlines, post-404 access revalidation, strict single/list response parsing, requested-repository/query-bound Link pagination, malformed/transport errors, an all-platform source-boundary guard, and an isolated Unix token-present read/list/verify/import process sentinel |
...
| Malformed issue-list item or pull-request marker | Entire listing fails closed; no default number/title/state is synthesized | Covered by `issue_list_strictly_parses_issues_and_rejects_malformed_entries` |
...
| Next link targets another repository or resource | Entire listing fails before another request | Covered by `link_header_rejects_wrong_repository_or_resource_path` |
