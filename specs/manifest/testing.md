...
| Single-project Gradle identity | conventional `src/main/kotlin/com/example/...` with `build.gradle.kts` and optional literal `rootProject.name` | `parse_gradle` / `compute_coverage_checked` | module is `rootProject.name` or the project directory; `com` and `src` are not modules |
...
| Composite build with a configuration block | `includeBuild("vendor/shared") { dependencySubstitution { … } }` beside `include(":app")`, one line and multi-line, with braces inside a string literal and inside a comment | `parse_gradle_settings` and a real `coverage` run over the fixture | `:app` is discovered with no manifest notice, the included build contributes no module and no source directory, and the escaping, interpolated, multi-argument, and unbalanced-block forms still fail closed |
...
| Trailing `includeBuild` configuration block | A balanced block is skipped and the declaration parses; the path in front of it is still confined, the block's own text is still judged by every other guard, and an unbalanced block or a non-block trailing expression fails closed | Exercise the reported multi-line form, one-line and Groovy spellings, braces inside strings and comments, `includeBuild("../outside") { … }`, interpolated and multi-argument forms, an unbalanced block, and governed directives written inside the block |
