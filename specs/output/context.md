...
- **Near-pure presentation layer**: `output` formats and prints data passed to it, and does no config loading and no process exit — those belong to the command layer (`src/commands/`). There is one file-I/O exception: `print_diff_markdown` re-reads the spec files it was handed, to compute the untracked-file list. Everything else is testable in isolation.
