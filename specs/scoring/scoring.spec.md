...
| `letter_grade` | `score: u32` | `&'static str` | The single grade-band table. Exported so consumers that render a grade beside a total — the MCP snapshot among them — call it rather than restating the bands, which is how one total came to carry two grades |
