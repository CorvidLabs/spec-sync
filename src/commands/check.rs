...
use super::{build_schema_columns, compute_exit_code, exit_with_status, load_and_discover, run_validation};
use crate::commands::issues::create_drift_issues;
...
pub fn cmd_check(
    ...
    let mut requirements_stale_specs: Vec<hash_cache::ChangeClassification> = Vec::new();
    ...
    for classification in &change_classifications {
        ...
        if classification.has(&hash_cache::ChangeKind::Requirements) {
        ...
        if classification.has(&hash_cache::ChangeKind::Companion) && matches!(format, Text) {
            println!(
