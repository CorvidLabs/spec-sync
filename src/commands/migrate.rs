...
fn check_detect_version(ctx: &MigrationContext) -> StepStatus {
    ...
    if !has_root_config && !has_new_config {
...
fn apply_write_gitignore(
    ...
    let content = "# spec-sync v4 — generated by `specsync migrate`\n\
                   # Committed: config.json, registry.toml, lifecycle/, changes/\n\
...
fn discover_specs(root: &Path) -> Vec<PathBuf> {
    // Try loading config to find specs_dir, fall back to "specs"
