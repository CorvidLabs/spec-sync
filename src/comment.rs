...
/// Leave headroom below GitHub's 65,536-byte comment limit for integrations
/// that wrap this body in a status summary or details element.
const COMMENT_MAX_BYTES: usize = 48 * 1024;
const COMMENT_TRUNCATION_NOTICE: &str = "\n\n---\n_Comment truncated to fit GitHub's size limit. Run `specsync check --format github` locally for the complete report._\n";

fn truncate_comment(mut comment: String) -> String {
    if comment.len() <= COMMENT_MAX_BYTES {
        return comment;
    }

    let prefix_limit = COMMENT_MAX_BYTES.saturating_sub(COMMENT_TRUNCATION_NOTICE.len());
    let mut boundary = prefix_limit.min(comment.len());
    while boundary > 0 && !comment.is_char_boundary(boundary) {
        boundary -= 1;
    }
    comment.truncate(boundary);
    comment.push_str(COMMENT_TRUNCATION_NOTICE);
    comment
}
...
pub fn render_check_comment(
    ...
    if !all_notices.is_empty() {
        out.push_str("### Planned Mappings\n\n");
        let grouped = group_by_spec(all_notices);
        for (spec, messages) in &grouped {
            let link = spec_link(spec, repo, branch);
            out.push_str(&format!("**{link}**\n\n"));
...
mod tests {
    ...
    #[test]
    fn test_render_check_comment_is_bounded_and_utf8_safe() {
        let coverage = CoverageReport {
        ...
        let errors = vec![format!(
            "specs/large.spec.md: {}",
            "Review this Unicode-safe failure 🦀 ".repeat(4_000)
        )];

        let output =
            render_check_comment(1, 0, 0, 1, &errors, &[], &[], &coverage, false, None, None);

        assert!(output.len() <= COMMENT_MAX_BYTES);
        assert!(output.is_char_boundary(output.len()));
        assert!(output.contains("Comment truncated to fit GitHub's size limit"));
        assert!(output.contains("specsync check --format github"));
    }
