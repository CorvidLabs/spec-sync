...
mod tests {
    ...
    #[test]
    fn test_learnxinyminutes_patched() {
        let src = std::fs::read_to_string("/private/tmp/claude-501/-Users-leif-Development--CorvidLabs-spec-sync/1429498c-236f-41e9-839d-cd71a8ca63b8/scratchpad/c_test/full_patched.c").unwrap();
        let symbols = extract_exports(&src);
        eprintln!("PATCHED AST SYMBOLS ({}): {:?}", symbols.len(), symbols);
    ...
    #[test]
    fn test_learnxinyminutes_bisect() {
        ...
        for n in (50..lines.len()).step_by(20) {
            let partial = lines[..n].join("\n");
            let tree = parse_c(&partial).unwrap();
            let root = tree.root_node();
            // find last top-level child and check if it spans to near the end
            let mut cursor = root.walk();
            let mut last_span = 0usize;
            for child in root.children(&mut cursor) {
                let span = child.end_position().row - child.start_position().row;
                if span > last_span {
                    last_span = span;
                }
            }
            eprintln!(
                "n={} has_error={} max_child_span={}",
                n,
                root.has_error(),
                last_span
            );
    ...
    #[test]
    fn test_learnxinyminutes_repro() {
        let src = std::fs::read_to_string("/private/tmp/claude-501/-Users-leif-Development--CorvidLabs-spec-sync/1429498c-236f-41e9-839d-cd71a8ca63b8/scratchpad/c_test/repro.c").unwrap();
        let symbols = extract_exports(&src);
        eprintln!("REPRO AST SYMBOLS ({}): {:?}", symbols.len(), symbols);
        let tree = parse_c(&src).unwrap();
    ...
    fn test_learnxinyminutes_scratch() {
        ...
        for child in root.children(&mut cursor) {
            eprintln!(
                "TOP CHILD: kind={} error={} start_line={} end_line={} text_prefix={:?}",
                child.kind(),
                child.is_error(),
                child.start_position().row + 1,
                child.end_position().row + 1,
                &child.utf8_text(src.as_bytes()).unwrap_or_default()[..child
