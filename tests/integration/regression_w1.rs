...
fn coverage_missing_referenced_file_exits_nonzero_by_default() {
    ...
    fs::write(root.join("src/auth/service.ts"), "export function login() {}\n").unwrap();
...
fn coverage_json_includes_missing_files() {
    ...
    assert!(
        missing.iter().any(|f| f.as_str().unwrap_or("").contains("ghost.ts")),
...
fn diff_head_fallback_prints_loud_stderr_warning() {
    ...
    let output = specsync()
        ...
        .output()
        .unwrap();
    assert!(output.status.success(), "diff fallback should still succeed");
